...
**Current authored-runtime status (2026-07-20):** all 19 current combat boards are authored: Takamine plus 18 regional boards. The party field foundation (6 × 8 directional idle/walk), combat-action keys (6 × 10, including recovery and defeat), and portrait expressions (6 × 8) are integrated, as are eight regular-enemy families with five key poses each (8 × 5, including defeat), ten bosses with six key poses each (10 × 6), and nine delivery/essence VFX with six phase frames each (9 × 6). The VFX overlay retains the procedural impact beneath it as a load/error fallback. These integrations remain foundation/key-pose coverage rather than complete packages: full in-betweens, alternate combat facings, variants, dedicated remaining enemy/boss recovery art, and remaining system effects are still absent. Human readability review, external cultural review, and Accepted/art lock remain pending.
