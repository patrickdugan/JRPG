...
* **Defence:** Guard is a pre-committed, one-pulse protective stance; in the combat proof it reduces the next documented hit by 45%. Dodge is likewise pre-committed, applies only to a documented dodgeable physical attack, and never asks for a real-time button press. Both are planning tools, not reaction buttons.
...
Status conditions are intentionally few: Bleed (damage on activation), Scorch (damage after command), Chill (−1 Pace next activation), Shock (cannot Guard), Dread (reduced healing received), and Bound (cannot move until hit or expiry). Bosses use telegraphed, cleanseable versions rather than surprise permanent locks.
