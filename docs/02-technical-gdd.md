...
Prototype fallback artwork remains original procedural pixel art. The current production replacements are deterministic editable pixel suites: all 19 combat boards; six-member 14-key field atlases with paired directional walk phases plus live interact/hurt keys, ten-key combat atlases, and eight-expression portrait atlases; eight regular-enemy families with defeat/recovery/hurt keys; ten primary bosses with transition/defeat/recovery keys; and nine battle-VFX families. Campaign currently renders Ren as the fixed field leader and therefore samples only Ren's paired walk keys; the other five authored pairs await a leader/formation authority. Generated concept sheets remain reference-only and are not shipped as runtime pixels. Remaining combat/scene in-betweens, alternate action facings, variants, and human review are still production work.

Two additional fail-closed foundations are live. A 96 × 192 status atlas provides distinct 32 × 32 apply/refreshed, persistent-active, and expire frames for Dread, Chill, Shock, Scorch, Bound, and Overheated; no campaign cleanse event exists, so no cleanse state is authored. Unknown statuses and Final Ward Open retain generic/special fallback. A 64 × 48 NPC atlas supplies only two 32 × 48 south-idle roles: exact side-story `talk + targetKind: person` resolves `speaker`, and exact scene-operation `interview` resolves `interviewee`. All other field markers stay geometric, with no ID, prose, name, or label inference.
