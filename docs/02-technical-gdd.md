...
Prototype fallback artwork remains original procedural pixel art. The current production replacements are deterministic editable pixel suites: all 19 combat boards; 20 opaque 320×180 Campaign scene panoramas; seven-member 14-key field atlases with paired directional walk phases plus live interact/hurt keys, ten-key combat atlases, and eight-expression portrait atlases; eight regular-enemy families with defeat/recovery/hurt keys; ten primary bosses with transition/defeat/recovery keys; and nine battle-VFX families. Campaign resolves its visible leader only from the active level's canonical formation and one field-v1 presentation flag, making all seven paired walk rows reachable without changing movement or battle formation. The same effective ID owns hazard vitals. Remaining members render from an ephemeral trail of successfully persisted departed tiles; the trail never enters collision, hazards, interaction range, saves, telemetry, or proof. A 19-frame transparent terrain atlas decorates the default floor plus every live authored tag across all 48 levels after the level-owned flat color; it never supplies collision. The separate scene-panorama registry maps all 60 exact beat IDs to one of 20 frames and accepts a frame only when the active field level belongs to that beat's explicit compatibility list. It lives outside campaign content, scene direction, field levels, encounters, saves, telemetry, and evidence; unknown or mismatched bindings and undecoded/wrong-size rasters retain the static code-native fallback. Generated concept sheets remain reference-only. The explicitly manifested portrait-v2 pipeline is the sole generated-source runtime exception: it ships a deterministic 96 × 96-cel pixelification with a shared 64-color visible palette, binary alpha, and exact provenance rather than claiming hand-pixel authorship. Remaining combat/scene in-betweens, alternate action facings, variants, whole-field map/tileset packages, and human review are still production work.
...
The side-story person contract is now explicit: only `type: 'talk'` plus `targetKind: 'person'` resolves `speaker` (4/71 objectives); the four `targetKind: 'group'` talks retain geometric markers. Party field art adds a second authored walk phase for every member and direction, sampled only during the existing 320 ms visual hold and suppressed to idle under reduced motion. Camp inventory/shop cards address all 25 live item IDs through an 80 × 80 icon atlas; Campaign Battle reuses River Salve's 16 × 16 frame at 2× in its Item preview and Canvas. Text remains authoritative on image failure.

## 2. First-playable boundary

The combat slice is **FP-0: Bell Court at Takamine**, a deliberately narrow proof before the planned 28–34 minute **FP-1 Takamine Vertical Slice**. Courier **Ren Ishikawa** reaches an abandoned temple gate with **Nikola Dražanić**, a Croatian-born fictional minor noble with English maternal ancestry, vampire hunter, and self-proclaimed Count whose title exceeds his actual standing. His house claims Wallachian origin and custody of the invented Covenant of the Severed Dragon, but its supposed male line repeatedly survives through heiresses and marriage contracts. A short in-game prelude makes clear that **Father Mateus Avelar**, the court’s vampire interpreter, has directed the search; Nikola recognizes a corrupted Dracul precedent and the counter-ward Mateus thought extinguished. They face an **Ashen Oni Tithe Enforcer**, an invented retainer of the Black Chrysanthemum court.
...
### Lineage and final political branch

The supernatural lineage is explicit alternate history. In 1462, Vlad III uses vampirism as an invented emergency weapon; the wholly fictional Covenant of the Severed Dragon answers by separating an immortal ruler's blood from public command. Later Dražanić hands corrupt that check into hereditary purge authority. Nikola inherits both the useful ward and the compromised claim around it. Mateus later translates the Dracul precedent for Kurozane, giving the hunter and priest shared technical knowledge without making either one the rightful owner of Japanese authority.

The final boss ends with Kurozane alive, ward-broken, mortal, and commandless enough for a post-boss Storyworld decision. **The Last Command** uses the accumulated reaction state to resolve one of two political consequences: a witnessed return of military, registry, granary, and bell seals under revocable binding, or execution/failed transfer followed by civil war as rival governors seize the regime's orphaned commands. The transfer is surrender under defeat, not absolution or restored honor. The civil-war branch is a consequence of the blood regime's deliberately irreplaceable structure, not a declaration that execution is inherently immoral. Neither branch appoints Nikola, Mateus, or the party as successor. Existing beat, encounter, party, and opaque Storyworld IDs remain stable, but the source identity changes: only prefixes before the revised Chapter 9 sequence migrate, while late old outcomes fail closed rather than acquire a new political meaning.
...
- Footer: command buttons with range/recovery labels, native `I` Item control, item and deployed-party target selectors, combat log, explicit keyboard controls, restart/end card.

Original procedural pixel primitives remain the resilient load/error fallback. Live presentation uses the authored board, scene-panorama, actor, portrait, boss, and VFX suites described above. Campaign's 320×180 panorama Canvas is a separate decorative 16:9 surface beneath the focus portrait and beside the existing location, atmosphere, dialogue, choice, and transcript authority; it is never stretched into the 960×480 field Canvas or used by Battle. Its exact beat/level binding, opaque frame crop, dimension gate, and failure fallback are presentation-only. Current key poses and panoramas validate silhouette, value, material, exact addressing, and deterministic fallback behavior; complete animation, native-resolution overlay readability, Narrator/NVDA, external cultural review, and human approval remain art-production deliverables.

Campaign Battle adds the authored six-status lifecycle overlay without joining command, animation, Recovery, terminal-hold, or settlement boundaries. Expiry lasts 520 ms at 1×, scales only with 2×/4× presentation speed, and becomes a static readable mark under reduced motion. Distinct victory and defeat board-edge accents are likewise result-gated and non-locking. A separate 720 ms typed damage-outcome channel accepts only exact appended `damage` events corroborated by sequential HP transitions and publishes exact damage/restoration plus WEAK, RESIST, IMMUNE, ABSORB, GUARD, WARD, and delivery/Essence percentages. It is shared by manual and Auto-Grind resolution but excluded from input, animation, playtime, settlement, reward, and queue boundaries. Campaign Dodge command presentation is bounded by the existing command timeline; its corroborated `dodge-resolved` feedback is a separate non-locking presentation channel and cannot become input, simulation, settlement, reward, or queue authority.

Campaign Item uses a separate 720 ms command record and exact `item-used` plus `heal` events. Only catalogue-opted River Salve is supported: HP effect 80, living deployed party target at any distance, no revive, base Recovery 2 with existing loadout/status modifiers and minimum 1, no Pace/Spirit change, and no stance mutation. An invalid, empty-stock, or zero-heal attempt leaves stock, logs, turn, Recovery, audio, and simulation untouched. Engine snapshot schema v2 exposes canonical immutable `itemStock` and `itemConsumption`. Heal feedback must corroborate catalogue, event order, pulse, actor/target, HP boundary, stock, and consumption; a later exact Scorch event may change final snapshot HP without rewriting the heal result. Item presentation plays one heal cue and never a hit cue. Attempt stock resets on defeat/restart/reload. Victory validates all item debits, rewards, and survivor HP before producing one new loadout revision; that state participates in the existing compensating persistence transaction, which is not a crash journal, multi-tab compare-and-swap, or guarantee against rollback failure. Auto-Grind never selects Item. Campaign still has no cleanse event, and unsupported MP/Spirit/status consumables remain Camp-only. None of these Campaign presentation rules alters the FP-0 seeded 65% Dodge rules in sections 2, 6, and 9.
