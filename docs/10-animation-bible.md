...
**Runtime note (2026-07-20):** original, editable, deterministic assets under `assets/art/` now replace the generated raster atlases in live rendering, and all 19 current combat boards are authored—Takamine plus 18 regional boards. Party rendering integrates the 6 × 8 directional field idle/walk foundation, 6 × 10 combat-action keys including recovery/defeat, and 6 × 8 portrait-expression keys. Enemy rendering integrates eight regular families × five key poses including defeat and ten bosses × six key poses. Battle VFX integrates nine delivery/essence effects × six authored 64 × 64 phase frames; the authored overlay remains above a procedural impact retained as the load/error fallback. Exact addressing, alpha gutters, source/export hashes, mappings, and fallback behavior are automated, but these remain key-pose foundations: full in-betweens, alternate combat facings, variants, dedicated remaining enemy/boss recovery art, and remaining system effects are absent. Human readability review, external cultural review, and Accepted/art lock remain pending.
