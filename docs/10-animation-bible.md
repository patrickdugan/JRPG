...
**Runtime note (2026-07-20):** original, editable, deterministic assets under `assets/art/` now replace generated raster atlases in live rendering, and all 19 combat boards are authored. The party field suite now contains 6 × 14 unique keys: directional idle/walk-A, a second walk-B phase for all four directions, and live interact/hurt. Campaign samples the paired walk keys for fixed field leader Ren; the other five authored pairs await a leader/formation authority. Combat remains 6 × 10 keys and portraits 6 × 8 expressions. Enemy rendering integrates eight regular families × seven poses and ten bosses × seven poses. Battle VFX integrates nine delivery/essence effects × six 64 × 64 phase frames plus six statuses × three 32 × 32 lifecycle frames; distinct board-edge victory/defeat accents are code-native presentation. Field rendering additionally has a metadata-only two-role NPC foundation, and Camp has 25 item icons. Exact addressing, gutters, hashes, mappings, dimension gates, and fallbacks are automated. Remaining combat/scene in-betweens, alternate action facings, variants, a full NPC roster, emitted Campaign heal, and Campaign Dodge remain absent. Human readability review, external cultural review, and Accepted/art lock remain pending.
