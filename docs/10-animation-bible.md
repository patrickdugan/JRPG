...
**Runtime note (2026-07-20):** original, editable, deterministic assets under `assets/art/` now replace the generated raster atlases in live rendering, and all 19 current combat boards are authored—Takamine plus 18 regional boards. Party rendering integrates the 6 × 8 directional field idle/walk foundation, 6 × 8 combat-action key poses, and 6 × 4 portrait-expression keys. Enemy rendering integrates eight regular families × four key poses and ten bosses × six key poses. Battle VFX integrates nine delivery/essence effects × six authored 64 × 64 phase frames; the authored overlay remains above a procedural impact retained as the load/error fallback. Exact addressing, alpha gutters, source/export hashes, mappings, and fallback behavior are automated, but these remain key-pose foundations: full in-betweens, recovery/defeat coverage, alternate combat facings, and the planned eight-expression speaking portrait set are absent. Human readability review, external cultural review, and Accepted/art lock remain pending.
