...
**Runtime note (2026-07-20):** three original, editable, deterministic suites under `assets/art/` now replace the generated raster atlases in live rendering. The party suite integrates six 32 × 48 rows and eight directional idle/walk foundation columns into Camp, field, scene-focus, and party battle rendering; full idle/walk in-betweens, interactions, hurt/guard/action/defeat clips, combat-scale bodies, and portraits remain unproduced. The enemy suite integrates eight live families with four 64 × 80 key poses each—neutral, wind-up, attack, and stagger—while movement/attack in-betweens, defeat clips, planned variants, portraits, and bespoke boss phases remain. The battle-VFX suite integrates nine delivery/essence effects with six authored 64 × 64 phase frames each; its overlay remains above the procedural impact, which is retained as a load/error fallback. Exact addressing, family mapping, alpha gutters, hashes, and fallback behavior are automated; human readability review and external cultural review remain pending before Accepted/art lock.
...
The keyframes, roster sheet, boss sheet, and superseded raster atlases in `assets/production/` remain generated **production references** for palette, silhouette, composition, and mood; their pixels are not runtime authority. The integrated party, enemy, and VFX suites in `assets/art/` have original editable sources, deterministic builders, transparent exports, manifests, and labeled review sheets, but they are foundation/key-pose packages rather than complete animation deliveries. Party action/portrait coverage, enemy in-betweens/defeats/variants/boss phases, and human readability plus external cultural review remain required before Accepted/art lock.
