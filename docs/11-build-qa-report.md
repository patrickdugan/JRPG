...
| Twenty-hour duration | Reference quantity target clears; measured target unproven | The all-finite quantity audit estimates 776.013 minutes low, 1,231.072 minutes reference, and 1,916.650 minutes high. The reference model clears 20 hours by 31.072 minutes. Estimates are not timed-play evidence, do not establish critical-path-only duration, and no complete human playthrough currently proves the target. |
...
| Unit and integration tests | `npm test` | Pass at milestone lock: 399/399 tests. Coverage includes combat presentation scheduling, every encounter/objective family, Spirit/status behavior, exact movement, scene-operation persistence/gates, canonical and finite completion, all three finite narrative catalogues and runners, run-bound compare-and-swap saves, rewards/replay refusal, duration evidence, routes, and world integrity. |
...
| Duration audit | `node --test tests/duration-audit.test.mjs` | Verifies that declared chapter budgets are excluded from evidence, quantity estimates never become proof, and only a valid timed run receipt can prove 20 hours. Current all-finite estimate: 776.013/1,231.072/1,916.650 minutes low/reference/high; reference surplus: 31.072 minutes. |
| Diff whitespace | `git diff --check` from repository root | Required before milestone lock. |
| Static delivery | Serve `game/` and request the release screens, styles, controllers, finite narrative packs/runtimes, and production rasters | Pass: 38/38 requested URLs returned HTTP 200, including all four screens, their styles/controllers, all companion/archive/council packs and runners, and five production raster assets. This does not prove DOM interaction, responsive layout, browser-console cleanliness, or visual quality. |
| Browser visual smoke | Headless Chromium initial render at 1440x1200 | Partial pass: the combat prototype and Camp rendered with their production sprites, controls, loadout, companion-conversation panel, and party-council panel. Full campaign/battle interaction, responsive review, console inspection, keyboard flow, and assistive-technology QA remain outstanding. |
...
9. The 37,717-word core script, 83,435-word companion catalogue, 27,506-word council catalogue, 31,163-word archive, and 60 scene operations are shipped runtime volume. The all-finite canonical-choice path exposes 76,547 companion words and 25,072 council words. Reading-speed and activity formulas can estimate that volume, but the honest audit remains 776.013/1,231.072/1,916.650 minutes at low/reference/high assumptions. The reference model clears 20 hours by 31.072 minutes; only a same-run human receipt can prove actual duration.

10. The original enemy atlas is a transparent 8x4 production raster used by the combat renderer, and attack timelines are deterministic. Headless visual smoke inspection confirms the combat prototype and camp render their initial production layouts. Interactive browser/canvas inspection, responsive review, visual comparison, input feel, console inspection, and assistive-technology testing remain outstanding.

## Next implementation gate

The integrated campaign is stable under the syntax gate, 399/399 milestone tests, 38/38 static delivery checks, and the two-screen headless visual smoke pass. The reference-model content gap is closed. Next, complete interactive browser, keyboard, responsive, console, assistive-technology, and cultural review; then run chapter-by-chapter and full clean-start human playtests. Tune from observed routes if necessary. The 20-hour target remains unproven until a valid normal-speed receipt records the complete player-facing run.
