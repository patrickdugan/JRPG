...
The integrated automated suite, syntax checks, and repo verifiers remain the deterministic gate; exact current counts and delivery bytes live in the QA report. The installed-Chrome probe verifies isolated movement/cooldown and Black Sun Concord behavior, then starts a fresh campaign run and wins Cinder Hounds through visible action inputs without a restart. Advancement, loadout, playtime, and run-receipt authorities exist, settlement reports `committed`, and Continue appears only afterward; isolated contexts retain empty storage and all console/page/HTTP error arrays remain empty.

The rendered-route verifier now defaults to canonical side-view action combat; `--legacy-battle` is the explicit tactical rollback. Its current recovery-linked receipt advances clean run `af41d14d` to 19/82 played scenes and clears six canonical action encounters through the Tithe Hound and the two Takamine approach fights. The Tithe boss clears in 20.11 seconds with zero restarts after multi-foe pressure, boss cadence, Camp preparation, and Aya's passive healing were made explicit. The verifier stops honestly at Father Mateus rather than claiming a full route. This is automated rendered-control evidence, not human balance/readability approval or a duration receipt; see `27-action-route-balance-receipt.md`.
