...
- First clears: 23/23 victories, 239 player commands, 96 enemy activations, zero party knockouts.
- Lowest surviving actor: 15.4545% HP in `fp1-cedar-path`; the checked lower bound is 10%.
- Highest command load: 50 in `c9-kurozane`; its 31 Guard commands are a 62% Spirit-recovery share under the deliberately aggressive policy, below the 75% audit ceiling.
- Replays: all 23 victories, 600 policy steps total, 348,280 ms of schedule-only 1× presentation, and exact decision/reward identity at 1×/2×/4×.
- Longest replay schedule: `c9-kurozane`, 91 steps and 42,760/21,380/10,690 ms at 1×/2×/4×, below the 125-step and 60,000 ms bounds.
...
Before this pass, Kurozane's generic first-in-range selection repeated only zero-power `Court Command`: 13 activations, 13 party damage, and none of the phase-specific Recovery grammar. The engine now selects a deterministic boss-phase kit: Court Command/Yearless Thrust alternate in `court`, Blood Eclipse owns `bell`, and Black Chrysanthemum owns `dawn`, with an ordinary legal-skill fallback. The current canonical trace uses all four skills, deals 118 party damage over seven activations, leaves the lowest member at 78.5507%, and exposes authored Recovery 1/2/3. Storyworld records remain encounter-briefing presentation only and never participate in this selection.

The current pass also corrected Ujiro's tribunal objective. Disabling four menu requirements previously ended the encounter after four player commands and one hostile activation, so the named boss never had to be subdued. `disableOrdersAndProtect` now requires the primary boss retreat as well as all authored objective actions. The deterministic trace uses four objective commands and ten skill commands across 18 player commands and three Ujiro activations, with a 22.2222% Guard share. Protection remains the tactical premise; depletion represents a forced retreat, not a lethal rewrite.

## Per-encounter receipt

| Encounter | Commands | Enemy acts | Minimum HP | KOs | Guards | Replay steps | Replay 1× ms |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `prologue-ashen-bailiff` | 3 | 3 | 75.4% | 0 | 2 | 16 | 10,680 |
...
| `c3-dock-patrol` | 2 | 3 | 100.0% | 0 | 0 | 5 | 5,560 |
...
| `c6-masked-clerks` | 12 | 4 | 96.8% | 0 | 8 | 16 | 12,120 |
| `c6-ujiro` | 18 | 3 | 99.3% | 0 | 4 | 38 | 18,440 |
| `c7-name-slip-release` | 2 | 2 | 91.1% | 0 | 0 | 11 | 6,360 |
| `c7-bell-warden-chiyo` | 3 | 1 | 93.4% | 0 | 0 | 4 | 4,440 |
| `c8-outer-court` | 2 | 3 | 100.0% | 0 | 0 | 5 | 5,560 |
| `c8-lady-enma` | 15 | 2 | 92.2% | 0 | 5 | 35 | 16,280 |
| `c9-archive-nodes` | 3 | 2 | 92.4% | 0 | 0 | 5 | 5,240 |
| `c9-yearless-bell` | 4 | 1 | 94.8% | 0 | 0 | 5 | 4,920 |
| `c9-kurozane` | 50 | 7 | 78.6% | 0 | 31 | 91 | 42,760 |
...
The canonical receipt changes are intentional: 239 player commands, 96 enemy activations, and signature `fnv1a32:3195bf45`. Updated low/reference/high projections are 193.920/320.552/516.883 canonical-only minutes, 230.630/405.000/710.473 canonical-plus-quest/chronicle minutes, and 784.726/1,244.604/1,937.305 all-finite minutes. The full-catalog narrative-plus-Storyworld reference is 341.969 minutes. None is observed duration evidence.
