...
That driver starts with the rendered New Game button, reads only published DOM text/`data-*` state, and mutates the game only through clicks, keys, Camp controls, and player-visible native prompt choices. Canonical action combat is the default; pass `--legacy-battle` only for the explicit tactical rollback. The current rendered-control receipt advances clean run `e8ee6b00` through all 18 opening beats, seven first clears, Mateus's phase-three surrender, and the required after-boss Storyworld consequence to the Chapter 3 handoff with zero restarts and empty browser-error arrays. Recovery import/export preserves continuity across fresh contexts, but every file remains `recoveryOnly: true` and `proofClaimed: false`. Add `--require-complete` and `--evidence-out` for an intentionally long full-route gate that must seal Credits and retain the downloaded evidence report.
