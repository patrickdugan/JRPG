...
Party field art now provides authored walk-A and walk-B keys for all six members and four directions. Campaign alternates Ren's pair during the existing 320 ms presentation hold because Ren remains the fixed field leader; the other five pairs are addressable assets pending a leader/formation authority. Reduced motion stays on the original idle frame, and field timing/collision/proof authority is unchanged. Camp inventory and shop cards use a complete 25-frame item-icon atlas while retaining item names, descriptions, quantities, prices, and contextual labels as the accessible authority and a text-only image-failure path.
...
The current release manifest contains 132 byte-verified files and 10,142,109 delivered bytes: five pages, six stylesheets, five controllers, 84 modules, 31 PNG production assets, and one SVG favicon.
