...
- **Campaign Battle (`battle.html?encounter=...`):** a shared multi-party engine for all authored encounters, including bounded Spirit, six live statuses, Tempo/recovery, Pace, typed damage, Guard, Dodge, Analyze, deterministic enemy AI, 18 objective presentations, nonlethal actions, XP, levels, rewards, equipped loadout modifiers, deterministic attack timelines, and repeat-only 1×/2×/4× Auto-Grind. Guard, Dodge, Analyze, and objective commands have bounded actor/target feedback; move, blocked-move, selected-target, real Recovery-lock attempts, and pulse-exact Tempo readiness use exact board geometry without adding input locks. Corroborated typed damage events publish exact damage/restoration flyouts with WEAK, RESIST, IMMUNE, ABSORB, GUARD, WARD, and delivery/Essence percentages through the same manual and automated seams. Party, regular-enemy, and boss recovery use authored poses; regular-enemy target staggers use authored non-gory hurt keys; and five bosses have typed phase state, exact warnings, and transition holds. FP-1 Mateus has dormant/active/broken Blood Wards, explicit 25% ward mitigation, a four-tile Crimson Litany answer window, Recovery 3, and a nonlethal surrender branch. Optional queues run 1, 5, or 10 wins; every victory and reward becomes durable before the next repeat starts, while cancellation, defeat, restart, or reload stops the session-only queue. Victory is durable before Continue appears: advancement, loot/vitals, requested quest or chronicle evidence, field resolution, and clean-run first-clear evidence share one compensating transaction.
...
The segmented route receipt above covers only the recorded `d45dfb2` runtime. It predates every later status, NPC, battle-feedback, walk-in-between, item-art, terrain, field-leader, level-metadata, telegraph-evasion, formation-follow, interaction-copy, and typed-damage-outcome wave; those waves pass their own full unit, static-delivery, and Chrome gates but have not inherited a segmented route claim. It also predates Campaign Dodge, which likewise does not inherit a segmented route claim. `durationProven` remains false.
...
The current release manifest contains 137 byte-verified files and 10,222,026 delivered bytes: five pages, six stylesheets, five controllers, 88 modules, 32 PNG production assets, and one SVG favicon.
...
| Command deck | Choose Attack, Skill, Guard, Dodge, Analyze, or the encounter-specific Objective action. |
...
The explicit 215-activity route targets 20 hours at 1×. Duration audit v8 excludes authored minute labels and open-ended repeats; it estimates the canonical story at 186.728/309.135/499.277 minutes and the complete intended route at 776.805/1,232.141/1,918.255 minutes low/reference/high. The intended-route reference model clears 20 hours by 32.141 minutes, while canonical-only reference play is about 5.15 hours. Only four measured repeat schedules accelerate, producing reference estimates of 1,231.834 minutes at 2× and 1,231.681 at 4×. This arithmetic is not observed playtime. The target remains unproven until one clean human run completes the route and records at least 20 active hours.

The bounded `canonical-run.mjs` audit proves mechanical completion without pretending to prove duration: one deterministic zero-time run covers 60 beats, 2,746 dialogue lines, 59 choices, 60/60 scene operations, 183/183 operation nodes, 34 route chains, 1,419 legal field moves, 236 interactions, 41 exits, 23 first clears, 231 player commands, 102 enemy activations, and 17 safehouse rests. Its stable signature is `fnv1a32:cafea1e3`; its run receipt is complete and correctly reports `durationProven: false`.
...
The additional live authored suites are a 96 × 192 status atlas containing 18 distinct 32 × 32 lifecycle frames, a 128 × 48 NPC foundation atlas containing four 32 × 48 south-idle role frames, and an 80 × 64 field-terrain atlas containing 19 transparent 16 × 16 material overlays. All are dimension-gated and retain geometric/generic/flat-color fallback behavior; none should be read as complete animation, full named-NPC coverage, or collision authority.
