...
The nonlinear narrative routes contain 74–77 played scenes and retain a five-hour observed active-play floor. The older 332.738-minute calculation combines all 60 authored campaign scenes with the longest visible path through all eleven Storyworld clusters; it is a full-catalog ceiling, not a selected-route duration estimate. The optional 215-activity route targets 20 hours at 1×. Duration audit v8 excludes authored minute labels and open-ended repeats; it estimates the authored canonical catalog at 190.129/314.253/507.008 minutes and the all-finite route at 780.934/1,238.306/1,927.430 minutes low/reference/high. Only four measured repeat schedules accelerate. All arithmetic is diagnostic; duration claims require human receipts.

The bounded `canonical-run.mjs` audit proves mechanical completion without pretending to prove duration: one deterministic zero-time run covers 60 beats, 2,789 dialogue lines, 59 choices, 60/60 scene operations, 185/185 operation nodes, 34 route chains, 1,429 legal field moves, 241 interactions, 41 exits, 23 first clears, 224 player commands, 95 enemy activations, and 19 safehouse rests. Its stable signature is `fnv1a32:2e7f5885`; its run receipt correctly reports `durationProven: false`.
