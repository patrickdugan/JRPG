...
- **FP-1 (opening through Mateus):** the default clean start now uses an 18-scene, roughly 1,800-word first-play cut spanning the Night Census, Aya and Nikola’s introductions, the learning battles, Takamine infiltration, and Mateus’s nonlethal surrender. The visible opening panel targets 30–45 active minutes and identifies the endpoint when the cells open. `campaign.html?extendedOpening=1` retains the longer compiled dialogue for production review. Automated reachability is evidence, not a substitute for the dedicated blind human gate in `docs/28-opening-slice-blind-playtest.md`.
...
That driver starts with the rendered New Game button, reads only published DOM text/`data-*` state, and mutates the game only through clicks, keys, Camp controls, and player-visible native prompt choices. Canonical action combat is the default; pass `--legacy-battle` only for the explicit tactical rollback. The current recovery-linked action receipt advances clean run `af41d14d` to 19/82 played scenes and six first clears through the Takamine approach, with empty console/page error arrays; it stops at the unresolved Father Mateus balance frontier. Recovery import/export preserves continuity across fresh contexts, but every file remains `recoveryOnly: true` and `proofClaimed: false`. Add `--require-complete` and `--evidence-out` for an intentionally long full-route gate that must seal Credits and retain the downloaded evidence report.
