...
The current release manifest contains 92 byte-verified files: five pages, six stylesheets, five controllers, 70 modules, five PNG production assets, and one SVG favicon.
...
The explicit 215-activity route targets 20 hours at 1×. Duration audit v8 excludes authored minute labels and open-ended repeats; it estimates the canonical story at 186.724/309.130/499.269 minutes and the complete intended route at 776.801/1,232.136/1,918.247 minutes low/reference/high. The intended-route reference model clears 20 hours by 32.136 minutes, while canonical-only reference play is about 5.15 hours. Only four measured repeat schedules accelerate, producing reference estimates of 1,231.829 minutes at 2× and 1,231.676 at 4×. This arithmetic is not observed playtime. The target remains unproven until one clean human run completes the route and records at least 20 active hours.

The bounded `canonical-run.mjs` audit proves mechanical completion without pretending to prove duration: one deterministic zero-time run covers 60 beats, 2,746 dialogue lines, 59 choices, 60/60 scene operations, 183/183 operation nodes, 34 route chains, 1,419 legal field moves, 236 interactions, 41 exits, 23 first clears, 231 player commands, 102 enemy activations, and 17 safehouse rests. Its stable signature is `fnv1a32:894f20e0`; its run receipt is complete and correctly reports `durationProven: false`.
