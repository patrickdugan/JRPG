...
That driver starts with the rendered New Game button, reads only published DOM text/`data-*` state, and mutates the game only through clicks, keys, and player-visible native prompt choices. A chained same-run receipt has reached 34/60 scenes, 91/215 route entries, 12/23 durable first clears, and 01:08:04 active play. Checkpoint 25 advanced through the repaired Kagura Furnace alternative-condition exit and recorded empty console/page error lists before exporting all thirteen authorities. A recovery checkpoint is not itself a complete route witness or independent duration proof. Add `--require-complete` only when running an intentionally long full-route gate.
