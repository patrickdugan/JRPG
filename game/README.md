...
`action-campaign-battle.html?encounter=c1-cinder-hounds` is the isolated Action Lab. It may clone current party, loadout, and run context as its starting condition, but victory, defeat, restart, reload, and return never write campaign storage. It deploys a strict two-fighter action loadout: Nikola + Mateus by default, or a selected pair through `&lead=ren&support=miyo` (accepted public IDs: `nikola`, `mateus`, `ren`, `aya`, and `miyo`). One fighter is directly controlled while the other uses deterministic companion AI; Aya uses the backline-support policy. Tagging preserves live physics and cooldown state and has a shared 240 ms recovery.

Campaign launches the same page with `mode=campaign`. In that mode, victory saves active time and atomically commits advancement, rewards, surviving duo HP, optional quest/chronicle evidence, field resolution, and clean-run first-clear evidence before Continue unlocks. All 18 authored objective families have side-view authority. Authored status, displacement, summon, weak-point, defensive-window, and boss-phase effects execute through deterministic adapters. `campaign.html?legacyBattle=1` selects the complete tactical controller as a sticky per-tab rollback; `legacyBattle=0` returns to action routing.

`action-slice.html` is the recommended playtest entry. It selects any two of Nikola, Mateus, Ren, and Miyo, then chains Cinder Hounds, Ash Wisps, Aya's one-use sanctuary, the Tithe Hound, and a short testimony consequence. Duo HP persists between encounters in `sessionStorage`; restart restores the current encounter checkpoint, reset removes only slice state, and no slice transition writes the canonical campaign save. Keyboard, touch, and standard gamepad controls are available.

The shared fixed-step foundation retains variable jump height, coyote time, jump buffering, cancel windows, lowered slide hurtboxes, moving maneuver hitboxes, and bounded air resources. Character profiles now specialize it: Nikola uses grounded Hunter Step and Radiance stake attacks; Mateus has a floatier jump and two-use Night Passage; Ren has the fastest Roofline kit with combat-only boundary wall cling/rebound; Miyo has Crosswind Step, Stormlift, Thunderfall, and one air lift. None of these traversal techniques gates level progression. When Nikola and Mateus are deployed, they can invoke Black Sun Concord within 180 px: Dawn Bolt remains a separate Arcane/Radiance hit, Penitent Night remains a separate Arcane/Umbral hit, both commit at the same kernel time, and each art starts only its normal level-scaled cooldown after animation. Action is canonical when Campaign supplies `mode=campaign`; an unqualified Action Lab URL remains session-local.

This folder contains the playable browser runtime, a player-facing title screen, and explicit developer training surfaces. The campaign, field, quest, battle, advancement, loadout, Storyworld, and playtime saves share one browser runtime. The ordinary 5–6 hour narrative target and optional 20-hour completionist target remain production claims until measured by end-to-end human playtests.

- **Training Court (`training.html?dev=1`):** the original Bell Court fixture validating integer-space movement, 2-Pace turns, Tempo/recovery, Guard/Dodge, delivery + essence resistance, deterministic Oni AI, victory/defeat, and restart.
- **FP-1 (opening through Mateus):** the default clean start now uses an 18-scene, roughly 1,800-word first-play cut spanning the Night Census, Aya and Nikola’s introductions, the learning battles, Takamine infiltration, and Mateus’s nonlethal surrender. Ren fights the early encounters while Aya heals from visible reserve; Nikola’s arrival introduces tag combat. An opening-only focus layout places the current story and contextual instruction before the field map while withholding later-campaign ledgers and telemetry. Its single contextual next-step control identifies and scrolls to the exact dialogue, field objective, story choice, encounter briefing, Storyworld consequence, next scene, or endpoint feedback control currently blocking progress. Each canonical opening first clear pauses at a visible **READY / Begin encounter** gate so the fight card can be read before enemies move; unavailable future-combo presentation remains hidden until its actual formation exists. The internal target remains 30–45 active minutes, and the player-facing completion state identifies the endpoint when the cells open. Double-click `..\PLAYTEST_OPENING.cmd` on Windows to serve and open a clean blind-test candidate. Launcher mode records build/run/timing/restart metadata, prevents accidental Chapter 3 entry at the endpoint, collects the protocol’s unprompted comprehension and experience responses, and downloads a `human-review-required` JSON receipt without exposing observer documentation during play. `campaign.html?extendedOpening=1` retains the longer compiled dialogue for production review. Automated reachability and synthetic feedback are evidence, not substitutes for the dedicated blind human gate in `docs/28-opening-slice-blind-playtest.md`.
- **Campaign (`campaign.html`):** all 11 chapters / 60 authored campaign scenes plus 40 authored Storyworld scenes form a 100-scene catalog. A selected route plays 76 scenes on Ash or 77 on Salt/Paper. The war table schedules three of four regional operations, skips the fourth, reorders the required Storyworld clusters, and instantiates `c8-02-consent-not-conscription` on the selected Salt, Ash, or Paper approach map. All routes now play eleven Storyworld decisions and eleven consequences because Sodegaura has its own four-option, three-reaction manifest cluster. Every Act III–IV decision reaction reads at least four prior threads and every consequence reaction reads four, converting consent, evidence, accountability, and care into evacuation, Oni disruption, bell intelligence, garrison defection, and succession capacity. The deterministic balance audit measures route depth and effect-vector geometry alongside distributions and inclination margins. Lady Enma's dedicated Cinder Fan spool follows her third recurring boss fight and records death, rotating custody, or bounded negotiated defection; custody/compact preserve bounded intelligence while death explicitly loses it. The final post-Kurozane cluster, **The Last Command**, resolves five tactics through thirteen dramatic flows as living abdication under mortal custody, witnessed seppuku after a public dawn transfer, a prepared execution with civil continuity, or an unprepared execution/failed parley followed by civil war. Kurozane’s pride, indispensability claim, and guilt pressure receive bounded historical nudges; only the rare public confession applies the exact pride complement. No ending appoints Nikola, Mateus, or the party as successor. The persistent campaign dialogue, 60 bespoke atmosphere/score/camera/blocking transcripts, and ordered once-per-save field operations remain intact. Every operation uses exact movement and authored instructions; encounter nodes bind the existing fight and cannot create a duplicate battle. The party field atlas supplies directional idle/walk plus live interaction and non-gory hazard-hit reactions; successful persisted movement leaves an ephemeral, collision-free trail for visible formation followers. Exact presentation copy covers all field interactables while save-stable IDs remain internal. New Game unlocks the complete authored Prologue roster, and each completed frontier atomically unlocks the next chapter's roster. A witness combat stage consumes an already-recorded victory instead of forcing a duplicate battle. The actionable 215-activity route ledger remains visible as optional completionist work and never blocks narrative advancement.
- **Acts IV–V level flow:** `content/late-act-level-design.mjs` validates the three route-specific Black Gate approaches, five-room Kurohana ascent, six exact Act V level visits, and the safe Outer Archive evacuation return. Five action-cutover arena variants bind the existing side-view stage geometry to the Outer Court, Lady Enma, archive-node release, Yearless Bell, and Kurozane encounters without adding a movement ability gate or required backtracking.
...
- **Tactical rollback (`battle.html?encounter=...`):** the prior shared multi-party engine remains complete for all authored encounters, including bounded Spirit, six live statuses, Tempo/recovery, Pace, typed damage, Guard, Dodge, Item, Analyze, deterministic enemy AI, 18 objective presentations, nonlethal actions, XP, levels, rewards, deterministic attack timelines, and repeat Auto-Grind. It retains the same atomic victory transaction as action combat and is selected through `campaign.html?legacyBattle=1`; it is a rollback/reference surface, not the default campaign route.
- **Camp & Loadout (`camp.html`):** party vitals, Spirit/status recovery, consumables, shops, buy/sell, gear, forge upgrades, two Vow slots per character, three camp-rest tiers, the authored seven-character/eight-expression portrait atlas with a procedural failure fallback, 90 finite two-person companion conversations, 30 multi-character party councils, and a 60-record public reading table tied to canonical story beats. Required reading supports `N` for one exact next-line transition and `1`/`2` for explicit choices; held-key repeats are ignored.
- **Synthesized audio:** all six browser surfaces expose the same labelled Sound toggle, volume range, and polite status. AudioContext creation remains behind the visible player gesture; exploration, battle, and boss loops plus transition cues are generated locally without fetched assets. The versioned device preference is intentionally excluded from New Game, the fourteen recovery authorities, playtime, receipts, and evidence exports.
- **Active-play telemetry:** narrative, exploration, first-clear battles, repeat grind, and camp/menu time accumulate in both a general record and a clean-start, UUID-bound run receipt. Samples suspend after 30 seconds without input. Campaign, Battle, Camp, and Credits attach every clean-run sample to an authored chapter; Credits renders the live five-category ledger plus actual/reference time and completed-chapter gap for all eleven chapters. Narrative New Game requires the exact selected schedule—54–55 campaign scenes, 22 played Storyworld scenes, and at least five observed active hours. The 215-activity/20-hour completionist contract is separate and optional. The 20:36:18.683 completionist checkpoints are quantity-model diagnostics, never observed proof. Signed schema-v3 evidence publishes separate narrative and completionist verdicts, reconciles Storyworld and receipt authorities, and requires zero unattributed time.
...
The NPC atlas contains 16 exact community roles with idle and gesture poses, for 32 deterministic runtime frames. Nine singleton level contacts and four named side-story contacts resolve explicit roles from authored metadata; four collective talk targets remain geometric. The 37/185 interview operations intentionally retain the generic interviewee presentation. The resolver never infers roles from IDs, prose, labels, or names. Reduced motion freezes gestures on idle. Witness fieldwork and props, hazards, exits, evidence, mechanisms, deliveries, care/rescue, councils, combat, and ambiguous markers retain geometric fallback; interviewee number badges remain visible. The generated 1448×1086 community roster in `assets/production` is a non-runtime external-review reference, not a shipped sprite authority.
...
Then open `http://localhost:8080/` for the title screen. Developer fixtures remain available at `http://localhost:8080/training.html?dev=1` and `http://localhost:8080/action-campaign-battle.html`. Stop the local server with `Ctrl+C` when finished.
...
The harness uses fresh Chromium contexts and an ephemeral localhost port. The prior integrated pass completed in 154 seconds on installed Chrome. It verifies New Game selects the narrative profile and exposes both authored Prologue members, a fresh one-click opening route action and Camp deep link, a real keyboard archive advance, Campaign/Camp receipt continuity, first-clear speed gating, one real `F`-selected Dodge with an armed cue and HP-neutral consumed miss, and one real `I`-selected River Salve heal with ready icon art, reload refund, victory-only deduction, and one settlement revision. The opening scene panorama must decode `ready` at exact 320×180; broken and wrong-size PNGs must retain the nonblank fallback, the browser-loaded registry must reject a noncanonical level, responsive scaling must remain 16:9, and reduced motion must remain static. It also proves saved 4× repeat speed and a complete Auto-Grind victory and reward without Item selection. A separate Kagura context performs one legal persisted field move and requires Aya to render as the first follower in the exact six-member formation. Another context renders the `sw3` pre-boss decision, proves movement/dialogue/battle/advance controls blocked, makes entry and consequence choices through player-facing controls, and requires the receipt and Storyworld authority to persist the same ordered prefix. The recovery context exports all 14 authorities, replaces the run, restores through the rendered import control, and compares every raw string exactly while retaining `durationProven: false`. The current scripted seed targets the 77-scene Salt route and must remain unsealed for `active-playtime-incomplete`; downloaded schema-v3 evidence retains false duration/release verdicts while the optional 215 ledger stays nonblocking. The full 180-entry Camp catalogue, exact 390×844 geometry, semantic gates, keyboard reachability, denied-storage startup, HTTP delivery, and empty console/page/HTTP error arrays are also covered without touching the player's browser profile. This bounded smoke does not claim a current full route or five-hour timing proof.

For the stricter rendered-control-only route probe, run:

```powershell
python tools/browser-route-playthrough.py --max-scenes 30 --max-seconds 900 --max-action-restarts 0 --stop-after-beat c3-01-separate-arrivals
```

That driver starts with the rendered New Game button, reads only published DOM text/`data-*` state, and mutates the game only through clicks, keys, Camp controls, and player-visible native prompt choices. Canonical action combat is the default; pass `--legacy-battle` only for the explicit tactical rollback. The current rendered-control receipt advances clean run `41236c32` through all 18 opening beats, seven first clears, Mateus's phase-three surrender, and the required after-boss Storyworld consequence, then returns `target-reached` at the exact published Chapter 3 handoff with zero restarts and empty browser-error arrays. Recovery import/export preserves continuity across fresh contexts, but every file remains `recoveryOnly: true` and `proofClaimed: false`. Add `--require-complete` and `--evidence-out` for an intentionally long full-route gate that must seal Credits and retain the downloaded evidence report.

To verify the blind-test endpoint itself through the full opening, use a separate output path instead of `--stop-after-beat`:

```powershell
python tools/browser-route-playthrough.py --max-scenes 30 --max-seconds 1200 --max-action-restarts 0 --opening-feedback-out opening-feedback-probe.json
```

The current run returned `opening-feedback-exported` after 18/18 scenes and seven zero-restart action battles. It filled and downloaded the visible form at 390×844 with no console/page errors. The synthetic receipt retains `verdict: "human-review-required"` and fails the human-duration check, as it must.

The older three-session receipt on runtime commit `d45dfb2` completed 60/60 scenes, 215/215 intended-route activities, and 23/23 durable first clears through the tactical controller on clean run `ec3182b2-73b3-4b83-90b1-79a740e036ed`, then explicitly sealed Credits at 00:28:13. Its [downloaded schema-v2 evidence](../docs/rendered-route-playtest-evidence.json) remains historical reachability evidence, not a claim about the current action route.
...
The current release manifest contains 162 byte-verified files and 10,818,847 delivered bytes: seven pages, eight stylesheets, seven controllers, 105 modules, 34 PNG production assets, and one SVG favicon.
...
The nonlinear narrative routes contain 76–77 played scenes and retain a five-hour observed active-play floor. The current 341.969-minute calculation combines all 60 authored campaign scenes with the longest visible path through all twelve Storyworld clusters; it is a full-catalog ceiling, not a selected-route duration estimate. The optional 215-activity route targets 20 hours at 1×. Duration audit v8 excludes authored minute labels and open-ended repeats; it estimates the authored canonical catalog at 193.920/320.552/516.883 minutes and the all-finite route at 784.726/1,244.604/1,937.305 minutes low/reference/high. Only four measured repeat schedules accelerate. All arithmetic is diagnostic; duration claims require human receipts.

The bounded `canonical-run.mjs` audit proves mechanical completion without pretending to prove duration: one deterministic zero-time run covers 60 beats, 2,849 dialogue lines, 59 choices, 60/60 scene operations, 185/185 operation nodes, 36 route chains, 1,454 legal field moves, 241 interactions, 42 exits, 23 first clears, 239 player commands, 96 enemy activations, and 19 safehouse rests. Its stable signature is `fnv1a32:3195bf45`; its run receipt correctly reports `durationProven: false`.
...
The chronological `required-route-run.mjs` witness combines the canonical and finite systems into the exact 215-activity itinerary. It completes 60 beats, 23 first clears, 13 quests, 18 chronicles, 90 talks, 30 councils, 60 archive records, and four genuine repeat wins through 7,599 public transitions. Its witness-fieldwork component starts 67 fresh authored map contexts and reaches all 152 ordered nodes through exactly 729 public `moveFieldBy` steps, with zero coordinate jumps. Contract signature: `fnv1a32:b7b98301`; fieldwork traversal signature: `fnv1a32:03290d06`; executable signature: `fnv1a32:a9ae88b8`. Its 1×/2×/4× repeat schedules are measured engine output with invariant decisions and rewards. It records zero elapsed time and makes no duration claim from deterministic traversal.

The `camp-conversation-run.mjs` witness completes all 90 finite companion talks in canonical order: 3,644 main-line acknowledgements, 270 selected response acknowledgements, 90 explicit choices, 4,094 successful transitions, and 90 replay refusals. The catalogue contains 83,460 authored words across both branches; the duration audit counts 76,572 words visible on its canonical first-choice path. Its prose-bound catalogue signature is `fnv1a32:a3a22f12`, its completion signature is `fnv1a32:83556fcd`, and it records zero elapsed time.

The `party-council-run.mjs` witness completes all 30 multi-character councils: 993 main-line acknowledgements, 90 selected response acknowledgements, 30 decisions, 1,143 successful transitions, and 30 replay refusals. Its 27,511-word catalogue exposes 25,077 words on the canonical first-choice path. Its prose-bound catalogue signature is `fnv1a32:7bd9b978`, its completion signature is `fnv1a32:d16a6687`, and it records zero elapsed time.

The `archive-record-run.mjs` witness reads all 60 beat-bound public records across 498 paragraphs, executes 558 successful transitions, records 619 trace events, and refuses all 60 replay attempts. Its prose-bound catalogue signature is `fnv1a32:4d0dd995`, its completion signature is `fnv1a32:600fc989`, and it records zero elapsed time.
...
All shipped visuals are original project assets or Canvas pixel primitives. The combat boards, scene panoramas, party field/combat atlases, regular enemies, boss key poses, and battle VFX remain editable deterministic code-native pixel art. The live seven-party portrait-v2 atlas is the explicitly manifested exception: it is a deterministic 96 × 96-cel pixelification of seven AI-generated, identity-locked expression boards, with one shared 64-color visible palette, binary transparency, exact source/output hashes, and no claim of hand-pixel authorship. Other generated concept sheets remain reference-only and are not runtime pixels. The battle renderer combines the authored suites with deterministic wind-up, exact-grid lunge, projectile/trail, impact, status, stagger, and recovery phases while retaining procedural load/error fallbacks. Scene panoramas remain beneath portraits/dialogue and never become field or battle geometry. The assets explicitly exclude real-person likenesses; no copied character names or franchise assets are used.
