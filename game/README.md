...
- **FP-1 (opening through Mateus):** the default clean start now uses an 18-scene, roughly 1,800-word first-play cut spanning the Night Census, Aya and Nikola’s introductions, the learning battles, Takamine infiltration, and Mateus’s nonlethal surrender. Ren fights the early encounters while Aya heals from visible reserve; Nikola’s arrival introduces tag combat. The visible opening panel targets 30–45 active minutes and identifies the endpoint when the cells open. Double-click `..\PLAYTEST_OPENING.cmd` on Windows to serve and open a clean blind-test candidate. `campaign.html?extendedOpening=1` retains the longer compiled dialogue for production review. Automated reachability is evidence, not a substitute for the dedicated blind human gate in `docs/28-opening-slice-blind-playtest.md`.
