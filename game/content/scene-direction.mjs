...
export const SCENE_DIRECTIONS = deepFreeze([
  ...
  scene('chapter-3', 'c3-04-lantern-boat-escort', {
    atmosphere: 'Rain docks glow in three route colors while Lady Enma\'s paper parasol and premature reflection hold the far piling.',
    musicCue: 'Three calm route variations acquire a dry fan-clap and one dissonant court string when Enma opens her first mask.',
    cameraCue: 'Survey each route from the witnesses’ position, reveal Enma inside a puddle before tilting to her body, then return ownership of the frame to the route board.',
    entranceCue: 'Aya carries the route board to the families; Enma enters only after the witnesses remain closest to every choice marker.',
    gestureCue: ['AYA', 'Keep the route board turned toward the witnesses as Enma opens the cinder fan, refusing to let the boss entrance take ownership of their choice.'],
    blockingCue: 'Place witnesses closest to route markers, the boat operator at the exit, and Enma beyond the combat lane so her arrival changes danger without taking the choice.',
    transitionCue: 'The selected lantern color fills the frame; an orange fan lane cuts through it and clears into the first-mask boss escort.',
  }),
  ...
  scene('chapter-6', 'c6-04-printmaker-flight', {
    atmosphere: 'Rain slicks roof tiles around a civilian courier while Enma\'s archive mask repeats itself in every wet surface.',
    musicCue: 'Fast roof percussion follows the courier; the Cinder Fan theme mirrors its last bar whenever Enma duplicates a clerk writ.',
    cameraCue: 'Track beside the courier, preserve landing tiles, and show each black clerk target before its separate violet reflection.',
    entranceCue: 'The courier climbs from the hatch; Enma turns a black-red parasol above the west ridge while two clerks close the other approaches.',
    gestureCue: ['NIKOLA', 'Point from the latest black clerk writ to its violet reflection before lowering the spear into the courier\'s protection line.'],
    blockingCue: 'Keep the courier on the protected inside line, Nikola between them and attackers, and Enma mobile outside the wet reflection weak point.',
    transitionCue: 'The courier descends beyond the last roof as Enma burns her second mask into a retreating cinder silhouette.',
  }),
  ...
  scene('chapter-9', 'c9-05-dawn-at-observatory', {
    atmosphere: 'First daylight cuts across the observatory’s last execution ring while Kurozane’s three shadows divide the floor into Bell, blood, and dawn lanes.',
    musicCue: 'Kurozane’s court pulse drives the argument until Nikola’s lance reveal introduces a hard three-note challenge; Mateus’s cipher answers in descending counterpoint, and both lock into the Black Sun Concord cadence.',
    cameraCue: 'Stage the confrontation as advancing and retreating duel distance: close for each ideological strike, return to tactical height for every ward read, and never let dialogue conceal the three-pulse recovery lane.',
    entranceCue: 'Kurozane unfolds from the command circle as Nikola strips the travel cloth from the Longinus point; Mateus enters at Nikola’s shoulder, irritated by both men before either speaks.',
    gestureCue: ['NIKOLA', 'Level the revealed lance with theatrical certainty; after Mateus corrects the boast, lower it one embarrassed degree before resuming guard.'],
    blockingCue: 'Nikola and Mateus hold the forward line without excluding Ren’s command position; Miyo owns the dawn count, Kiku and Genta own extraction, and Aya keeps the archive route visible behind the duel.',
    transitionCue: 'Black Sun Concord cracks the ward into a sunrise lane; after the third recovery window, match Kurozane’s fall to the Longinus point stopping beneath his jaw rather than entering it.',
  }),
