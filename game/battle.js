...
import { createBattleResultFromSnapshot } from './battle-result-contract.mjs';
...
function drawBattle(now = performance.now()) {
  ...
  for (const actor of resolvedPresentationActors) {
    ...
    const centerY = originY + renderTile.y * cell + cell / 2 + Math.sin(visualNow / 170 + actor.pos.x) * Math.max(1, cell * 0.035);
...
function createCombatantCard(actor, selected = false, targetable = true, partyCommandAttempt = false, itemTarget = false) {
  ...
  meta.textContent = `Space ${actor.pos.x},${actor.pos.y} · ${actor.stance.toUpperCase()} · ready pulse ${actor.readyAtPulse}${actor.analyzed ? ' · ANALYZED' : ''}${statuses.length ? ` · ${statuses.join(', ')}` : ''}`;
...
function renderCommands(snapshot) {
  ...
  if (objectiveButton) objectiveButton.textContent = `6 · ${pendingObjective ? objectivePresentation.buttonLabel : 'Objective'}`;
...
function announceEngineLogDelta(beforeSnapshot, afterSnapshot) {
...
function recordVictoryIfNeeded(snapshot) {
  ...
  let resultRecord;
  try {
    resultRecord = createBattleResultFromSnapshot(snapshot, encounter.id);
  } catch (error) {
    return failSettlement(`Victory is earned, but its terminal result was rejected: ${error.message}`);
  }
  const settlement = settleBattleVictory({
    ...
    handoff: {
      ...
      questObjectiveId: requestedQuestObjectiveId,
      fieldTriggerId: requestedFieldTriggerId,
      chronicleId: requestedChronicleId,
      chronicleStageId: requestedChronicleStageId,
      chronicleChoiceId: requestedChronicleChoiceId,
    },
    flushPlaytime: () => {
      const ok = flushRunReceiptPlaytime();
      return { ok, state: runReceiptState };
    },
  });
  if (!settlement.ok) return failSettlement(settlement.message);
  ...
  settlement.messages.forEach(addMessage);
...
window.addEventListener('keydown', (event) => {
  ...
  if (commandShortcuts[key] && !event.repeat) {
    ...
    selectCommand(commandShortcuts[key]);
    return;
  }
  if (event.key === 'Enter' && !event.repeat && !confirmCommand.disabled) {
    event.preventDefault();
    confirmCommand.click();
    return;
  }
  const directions = {
...
reducedMotion.addEventListener('change', () => drawBattle(performance.now()));
