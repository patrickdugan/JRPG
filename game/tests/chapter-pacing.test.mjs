...
test('checked-in chapter checkpoints exactly match the quantity-derived audit', () => {
  ...
  assert.equal(CHAPTER_PACING_CHECKPOINTS.signature, 'fnv1a32:dab8e7de');
...
test('chapter checkpoints cover the canonical campaign and reconcile to the reference intended route', () => {
  ...
  assert.equal(CHAPTER_PACING_CHECKPOINTS.aggregateTargetMs, 73_928_467);
  assert.equal(CHAPTER_PACING_CHECKPOINTS.aggregateTargetMinutes, 1_232.141);
  assert.equal(audit.reconciliation.durationAuditEstimatedMinutes, 1_232.141);
...
test('pacing checkpoints remain diagnostics built without authored minutes or fabricated elapsed time', async () => {
  ...
  assert.equal(audit.reconciliation.canonicalSignature, 'fnv1a32:cafea1e3');
