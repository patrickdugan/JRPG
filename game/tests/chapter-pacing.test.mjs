...
test('checked-in chapter checkpoints exactly match the quantity-derived audit', () => {
  ...
  assert.equal(CHAPTER_PACING_CHECKPOINTS.signature, 'fnv1a32:8107b2cd');
...
test('chapter checkpoints cover the canonical campaign and reconcile to the reference intended route', () => {
  ...
  assert.equal(CHAPTER_PACING_CHECKPOINTS.aggregateTargetMs, 73_901_133);
  assert.equal(CHAPTER_PACING_CHECKPOINTS.aggregateTargetMinutes, 1_231.686);
  assert.equal(audit.reconciliation.durationAuditEstimatedMinutes, 1_231.686);
...
test('pacing checkpoints remain diagnostics built without authored minutes or fabricated elapsed time', async () => {
  ...
  assert.equal(audit.reconciliation.canonicalSignature, 'fnv1a32:029392d1');
