...
Release verification currently covers byte-exact localhost delivery for 122 shipped browser files, with the current full-suite count recorded in the QA report. All 19 combat boards now use editable, manifested, deterministic pixel art: the 384 x 224 Takamine Bell-Chamber board plus 18 regional boards. Battle rendering and pointer input retain one exact 64-pixel-cell transform, and image failure falls back to procedural terrain. Deterministic editable runtime art also covers six party members' directional field foundations, combat key poses, and four-expression portraits; eight regular-enemy families' key poses; ten primary bosses' key poses; and nine delivery/essence VFX families with six frames each. Complete in-betweens, party/enemy defeat coverage, alternate combat facings, the planned eight-expression speaking set, variants, remaining system effects, and subjective/cultural review remain production gates. The five browser surfaces share an autoplay-safe synthesized score and cue layer with visible sound/volume controls, resilient suspended-context recovery, and a device preference kept outside run proof and recovery authorities. FP-1 Mateus exposes dormant/active/broken Blood Wards, 25% ward mitigation, a four-tile Crimson Litany answer window, Recovery 3, and nonlethal surrender. The isolated Chrome flow covers the authored Prologue party from New Game, a five-win 4× Auto-Grind queue with five separately durable rewards, authored-art ready/error and reduced-motion paths, a fresh one-click route entry, persisted keyboard archive advance, cross-page persistence, all 180 Camp catalogue entries completed through their rendered controls, the 215/215 credits gate, eleven actual/reference chapter checkpoints, sealed-report download with zero unattributed time, and a 13-authority recovery checkpoint exported, replaced by another run, restored, reloaded, and compared byte-for-byte. The same pass covers exact 390 px no-overflow checks, skip-link and focus-retention behavior, touch-only FP-0 completion, reduced-motion canvas stability, slider-key isolation, duplicate-ID/access-name/image-alt semantic gates across all five pages, denied-storage startup, keyboard reachability, and zero console/page/HTTP errors. The archived rendered-control-only route from commit `630ef46`—with no direct storage access or runtime transition calls—completed 60/60 scenes, 215/215 intended-route activities, and 23/23 durable first clears on clean run `0a3402eb-e8a4-4dba-956b-67b0438400a2`, then explicitly sealed Credits at 01:32:45. Its [downloaded schema-v2 evidence](docs/rendered-route-playtest-evidence.json) has signature `fnv1a32:f8d659a2`, SHA-256 `E07B088BCB00E834485C52E177C753DE3F9AAB937F29C5D0DA12D64EC749C279`, and correctly reports the duration target as unproven; every chained browser segment recorded empty console/page error arrays. It remains prior-build reachability evidence because this milestone changed canonical content and pacing signatures; a fresh full segmented route has not yet replaced it. Recovery files preserved the archived run across fresh browser contexts through rendered import/export controls, but remain continuity-only artifacts labeled `recoveryOnly: true` and `proofClaimed: false`; the cumulative rendered-control logs and final signed evidence are that prior build's route witness, not the checkpoint files themselves.
