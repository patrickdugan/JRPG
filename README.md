...
Combat is now pivoting from the shipped turn-based tactical foundation to side-view real-time action. The new [action-combat feel proof](game/action-battle-prototype.html) provides free running and grounded jumping, fixed attack commitments, a separate shared weapon cooldown, a longer art cooldown, level-scaled timer recovery, typed damage, deterministic enemy pressure, and the existing side-facing project sprites. The DOM-free action kernel is fixed-step and independently tested. This proof is intentionally non-canonical until the existing atomic victory settlement and all encounter/objective adapters move behind an engine-independent contract; the complete turn-based Campaign Battle remains the current route authority during that migration.
...
Run [`game/`](game/README.md) and open `http://localhost:8080/action-battle-prototype.html` for the side-view action feel proof, `http://localhost:8080/campaign.html` for the Campaign Atlas, `http://localhost:8080/camp.html` for Camp & Loadout, or `http://localhost:8080/credits.html` for the explicit end-of-run credits boundary. Production-reference art and its reproducible prompt record are in [`assets/production/`](assets/production/README.md).
