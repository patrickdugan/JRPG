...
Release verification covers 766/766 tests and byte-exact localhost delivery of 130 browser files: 5 HTML, 6 CSS, 5 JS, 83 MJS, 30 PNG, and 1 SVG totaling 10,129,575 bytes. All 19 combat boards and the party field/combat/portrait, regular-enemy, boss, and nine-family delivery/essence VFX suites are deterministic, editable, manifested, dimension-gated pixel art with procedural/generic fallbacks. The current wave adds 18 distinct 32 × 32 status frames—Dread, Chill, Shock, Scorch, Bound, and Overheated across apply/refreshed, persistent-active, and expire—and a metadata-only two-role NPC field foundation for exact side-story speakers and scene-operation interviewees. Every other field-marker class remains geometric; this is not a full named-NPC package.

Battle feedback remains presentation-only: movement, blocked cells, selected targets, real Recovery locks, Tempo readiness, Guard, Analyze, objectives, authored status lifecycles, actor recovery/defeat poses, and a victory border/corner accent do not change simulation or input authority. The explicit-heal seam is dormant because Campaign emits no heal resolution; positive absorption remains hit feedback. Campaign Dodge and a distinct defeat-canvas accent remain absent, as do full animation in-betweens, alternate action facings, a full NPC roster, and subjective/cultural review.
