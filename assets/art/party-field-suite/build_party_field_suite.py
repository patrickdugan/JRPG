...
def build_files() -> dict[str, bytes]:
    ...
    identity_contracts = {
        entry["id"]: {
            key: entry[key]
            for key in ("name", "legacyCompatibilityId", "origin", "lineage")
            if key in entry
        }
    ...
    manifest = {
        ...
        "paletteIds": palettes,
        "characterIdentity": identity_contracts,
        "frames": frames,
        "sources": [
    ...
    readme = readme.replace(
        "an original male Croatian minor aristocrat in historically grounded 1622 rain-travel layers",
        "a Croatian-born frontier minor aristocrat with English ancestry through his fictional mother Margaret Wychmere, dressed in historically grounded 1622 rain-travel layers. His house claims a Wallachian covenant line repeatedly transmitted through noblewomen and marriage contracts; that lineage is entirely alternate-history fiction and makes no real-world claim that vampires, vampire hunters, or the Covenant existed",
    ).replace(
        "the Covenant existed; its row ID",
        "the Covenant existed. Nikola's row ID",
    ).replace(
