...
These original, project-bound PNG files were generated with the built-in image-generation workflow. The three 2026-07-16 images are opaque production references. The 2026-07-18 party and enemy atlases are provisional transparent runtime assets used by Camp, field movement, scene focus, and battle tokens. Runtime source rectangles now use integer cells and 4px safety insets, but transparent-gutter regeneration and hand-authored frame cleanup remain open.
...
Constraints: invented fantasy enemies only; Ashen Oni are a deliberate fictional synthesis under a contested in-world label and must not copy a sacred figure, real ceremonial mask, or identifiable local tradition; no gore; no skull pile; no human ethnicity coded as monstrous; no copyrighted character, logo, lettering, readable symbols, watermark, celebrity likeness, franchise visual imitation, or text.
```
...
Primary request: Create an original 4:3 pixel-art field-scene keyframe for the FP-00 Rain Gate: a mountain bell temple under curfew in alternate Genna 8 (1622) Japan. Takamine’s registry is Kurozane’s fictional local experiment, not the later nationwide temple-certification system. The image must guide modular environment construction, navigation readability, rain layering, and the first playable's somber story tone; it is a reference, not a shippable map.
